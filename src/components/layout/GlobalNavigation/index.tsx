...
export const GlobalNavigation = ({ name, email, myPageUrl, avatarUrl, onLogout }: GlobalNavigationProps) => {
  return (
    <nav className="fixed right-0 bottom-0 left-0 z-50 border-t border-gray-200 bg-white backdrop-blur-sm md:top-0 md:right-auto md:left-0 md:h-screen md:w-16 md:border-t-0 md:border-r md:px-2 md:py-6 lg:w-64 lg:px-4">
        ...
        {menuItems.map(item => {
          ...
          return (
            ...
            >
              ...
              <span className="hidden text-sm font-medium text-gray-700 lg:inline">{item.label}</span>
            </Link>
          )
        })}
